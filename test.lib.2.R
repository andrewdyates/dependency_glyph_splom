...
RR <- splom(BC0.cls, BC0.dcor, MIN=0.1, asGlyphs=T)
RR <- splom(BC0.cls, BC0.dcor, MIN=0.1, asGlyphs=T, lwd=2)
dev.off()

pdf("gibbs.BC0.gsplom.transpose.pdf", height=10, width=14.68)
R <- splom(t(BC0.cls), t(BC0.dcor), MIN=0.1)
RR <- splom(t(BC0.cls), t(BC0.dcor), MIN=0.1, asGlyphs=T)
dev.off()
